...
function internalError(baseHeaders, requestId) {
  return json(500, { error: "INTERNAL_ERROR", code: "WORKER_EXCEPTION", details: null, requestId }, baseHeaders);
}
...
export default async function handleFetch(request, env, cfctx) {
  ...
  try {
    ...
    try {
      // Best-effort audit (do not throw from error handling)
      const safeMsg = (e && e.message) ? String(e.message).slice(0, 300) : "unknown";
      emitAudit(
        { requestId, session: { isAuthenticated: false, token: null }, tenantId: null },
        {
          eventCategory: "SYSTEM",
          eventType: "WORKER_EXCEPTION",
          objectType: "worker",
          objectId: null,
          decision: "SYSTEM",
          reasonCode: "EXCEPTION",
          factsSnapshot: { requestId, message: safeMsg },
        }
      );
    } catch {}
    return internalError(baseHeaders, requestId);
  }
}
